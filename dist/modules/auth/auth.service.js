...
class AuthService {
    ...
    resendOtp = async (req, res, next) => {
        ...
        const user = await this._userRepo.findOne({ filter: { email, confrimed: { $exists: false }, provider: user_enum_1.ProviderEnum.local } });
        if (!user) {
            throw new global_error_handler_1.AppError("User not exist or already confirmed", 400);
        }
    ...
    forgetPassword = async (req, res, next) => {
        const { email } = req.body;
        const user = await this._userRepo.findOne({
            filter: { email, isConfirmed: { $exists: true }, provider: user_enum_1.ProviderEnum.local }
        });
        if (!user) {
    ...
    resetPassword = async (req, res, next) => {
        ...
        const otpValue = await redis_service_2.default.getValue(redis_service_2.default.otp_key({ email, subject: email_enum_1.EmailEnum.forgetPassword }));
        if (!otpValue) {
        ...
        const user = await this._userRepo.findOneAndUpdate({
            filter: { email, confirmed: { $exists: true }, provider: user_enum_1.ProviderEnum.local },
            update: {
                password: (0, hash_1.Hash)({ plain_text: password }),
        ...
        await redis_service_2.default.deletekey(redis_service_2.default.otp_key({ email, subject: email_enum_1.EmailEnum.forgetPassword }));
        (0, response_success_1.successResponse)({ res });
    };
